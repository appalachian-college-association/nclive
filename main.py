...
def run_search(query, token, restrict_to_library=RESTRICT_TO_LIBRARY):
    """Run search in OCLC Discovery API"""
    headers = {
...
def _extract_record_fields(record):
    ...
    return (oclc, clean_text_for_export(title), clean_text_for_export(isns),
            general_format, specific_format,
            format_description, is_electronic_video, material_types_str)
...
def write_to_csv(data, output_file, lookup_id):
    ...
    with open(output_file, "a", newline="", encoding="utf-8") as csvfile:
        ...
        for record in records:
            ...
            writer.writerow([lookup_id, *fields])
            results_written += 1
...
def main():
    ...
    for i, (lookup_id, search_query) in enumerate(search_terms):
        ...
        query = f"x4:digital AND ({search_query})"

        try:
            data = run_search(query, token, restrict_to_library=RESTRICT_TO_LIBRARY)
            print(f"  Found {len(data.get('briefRecords', []))} records")
    ...
    try:
        with open(output_file, 'r', encoding='utf-8') as f:
            ...
            if row_count != total_records:
                print(
                    f"Warning: Expected {total_records} rows"
                    f" but found {row_count} rows in the CSV."
                    )
    except OSError as e:
