# clean_marc.py
"""Prepare NC LIVE (INFOBASE) MARC Data for pipeline"""
...
def clean_record_fields(record):
    """Return clean URLs (856), OCNs (035), and Publisher IDs (028)"""
    return [
...
def verify_processed_files(input_dir):
    ...
    processed_files = list(Path(input_dir).glob("*_processed.mrc"))

    if len(processed_files) == 2:
        print("Verification passed: Found exactly 2 processed files")
...
def process_all_marc_files(input_dir):
    ...
    archived_dir = Path(input_dir) / "archived"

    print(f"Processing MARC files in: {Path(input_dir)}")
    print(f"Archive directory: {archived_dir}")

    # Find all .mrc files that aren't already processed
    marc_files = (
        [f for f in Path(input_dir).glob("*.mrc")
         if not f.name.endswith("_processed.mrc")]
    )
    ...
    for marc_file in marc_files:
        ...
        output_path = Path(input_dir) / f"{name}_processed{ext}"
        ...
        try:
            ...
            with open(input_path_full, 'rb') as infile, open(output_path, 'wb') as outfile:
                ...
                record_count = 0
                for record in MARCReader(infile):
                    if record is not None:  # Handle potential None records
                        record.fields = clean_record_fields(record)
                        MARCWriter(outfile).write(record)
                        record_count += 1

                MARCWriter(outfile).close()

            print(f"Created: {name}_processed{ext} ({record_count} records)")
