...
KB_METADATA = "http://worldcat.org/webservices/kb/rest/"
KB_EXPORT = "http://worldcat.org/webservices/kb/export/"
...
def get_collection_details(api_key, collection_uid):
    ...
    collection_url = f"{KB_METADATA}collections/{collection_uid}"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if owner_id:
                kbart_url = (
                    f"{KB_EXPORT}{owner_id}/{owner_id}_{collection_uid}_kbart.txt"
                    )
...
def download_kbart_file(
    ...
    try:
        ...
        if response.status_code == 200:
            # Save the KBART file
            filepath = os.path.join(output_dir, f"{collection_uid}_kbart.txt")
            ...
            is_valid, _, validation_message = validate_kbart_freshness(
                metadata_count,
                filepath,
                collection_uid
            )
...
def _find_column_indices(headers):
    indices = {
        k: None for k in (
            'title_url', 'title_id', 'oclc_number', 'oclc_entry_id', 'publication_title'
        )
    }
    for i, header in enumerate(headers):
        h = header.lower()
        if 'title_url' in h or ('title' in h and 'url' in h):
            indices['title_url'] = i
        elif 'title_id' in h:
            indices['title_id'] = i
        elif 'oclc_number' in h:
            indices['oclc_number'] = i
        elif 'oclc_entry_id' in h:
            indices['oclc_entry_id'] = i
...
def parse_kbart_file(filepath):
    ...
    try:
        ...
        col = _find_column_indices(headers)
        print(f"Key column indices - title_url: {col['title_url']}, title_id: {col['title_id']}, "
              f"oclc_number: {col['oclc_number']}, oclc_entry_id: {col['oclc_entry_id']}")
        ...
        for line_num, line in enumerate(lines[data_start:], data_start + 1):
            ...
            record = {
                key: (
                    row_data[col[key]]
                    if col[key] is not None and col[key] < len(row_data)
                    else ''
                )
                for key in (
                    'title_url', 'title_id', 'oclc_number', 'oclc_entry_id', 'publication_title'
...
def validate_kbart_freshness(metadata_count, kbart_filepath, collection_name):
    ...
    try:
        ...
        tolerance = 0  # Allow small variance for processing differences if >= 1
...
def save_comparison_report(
        fod_comparison, jfk_comparison, output_file="kbart_comparison_report.txt"):
    ...
    report_lines = [
        ...
        "",
        "SUMMARY:",
        "Films on Demand (FOD) Collections:",
    ...
    for comparison in [fod_comparison, jfk_comparison]:
        ...
        if comparison['field_mismatches']:
            ...
            if len(comparison['field_mismatches']) > 20:
                report_lines.append(
                    f"... and {len(comparison['field_mismatches']) - 20} more field mismatches")
                report_lines.append("")
...
def organize_kbart_files_for_pipeline():
    ...
    """
    print("ORGANIZING FILES FOR PIPELINE")
    print("=" * 40)
    ...
    print("kbart_files/")
    print("   fod_kbart.txt (FOD working copy)")
    print("   jfk_kbart.txt (JFK working copy)")
    print("   kbart_comparison_report.txt")
    print("   originals/")
